$(document).ready(function() {
    ...
    $('.mobile-btn .open').click(function() {
        ...
        $('.mobile-btn .close').css({
            'opacity': '1' // اضافه کردن opacity
        });
    ...
    $(document).on('click', function(e) {
        if ($('.overlay').is(':visible')) {
            // اگر کلیک روی overlay بود یا روی عنصری خارج از منو
            if ($(e.target).closest('header .nav-section .menu-container').length === 0 && !$(e.target).hasClass('open')) {
                closeMenu();
            }
        }
    });
    ...
    function closeMenu() {
        ...
        $('.mobile-btn .close').css({
            'opacity': '0' // reset opacity when hiding
        });
